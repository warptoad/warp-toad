...
<div class="dark min-h-screen bg-background text-foreground">
				...
				<div class="hidden md:flex items-center gap-3">
					{#if walletStore.isEVMConnected}
						<Badge variant="secondary" class="font-mono text-xs">
							EVM: {walletStore.formatAddress(walletStore.wallets.evm)}
						</Badge>
					{/if}
					{#if walletStore.isAztecConnected}
						<Badge variant="secondary" class="font-mono text-xs">
							Aztec: {walletStore.formatAddress(walletStore.wallets.aztec)}
						</Badge>
					{/if}
					<Button onclick={openWalletDialog}>
						{walletStore.isBothConnected ? 'Manage Wallets' : 'Connect Wallet'}
					</Button>
				</div>

				<!-- Mobile: Hamburger menu -->
				<div class="md:hidden">
					<Button size="icon" variant="ghost" onclick={() => mobileMenuOpen = true}>
						<Menu class="size-5" />
					</Button>
				</div>
				...
				<Sheet bind:open={mobileMenuOpen}>
					<SheetContent side="right" class="w-[300px]">
						...
						<div class="py-6 space-y-4">
							<!-- Wallet Status -->
							<div class="space-y-3">
								<h3 class="text-sm font-semibold text-muted-foreground">Wallet Status</h3>
								
								{#if walletStore.isEVMConnected}
									<div class="p-3 border rounded-lg">
										<div class="text-xs text-muted-foreground mb-1">EVM Wallet</div>
										<div class="font-mono text-sm break-all">
											{walletStore.wallets.evm}
										</div>
									</div>
								{:else}
									<div class="p-3 border rounded-lg border-dashed">
										<div class="text-sm text-muted-foreground">EVM wallet not connected</div>
									</div>
								{/if}

								{#if walletStore.isAztecConnected}
									<div class="p-3 border rounded-lg">
										<div class="text-xs text-muted-foreground mb-1">Aztec Wallet</div>
										<div class="font-mono text-sm break-all">
								...
								{:else}
									<div class="p-3 border rounded-lg border-dashed">
										<div class="text-sm text-muted-foreground">Aztec wallet not connected</div>
									</div>
								{/if}
							</div>
	...
	<main class="container mx-auto px-4 py-6 md:py-8 max-w-4xl">
		<Tabs bind:value={uiStore.activeTab} class="w-full">
			<TabsList class="grid w-full grid-cols-2 mb-6 md:mb-8">
