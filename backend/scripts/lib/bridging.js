...
import { L1AztecBridgeAdapter__factory, IL1ScrollMessenger__factory } from "../../typechain-types";
...
export async function getLocalRootProviders(chainId) {
...
export async function getPayableGigaRootRecipients(chainId) {
    const contracts = evmDeployments[Number(chainId)];
...
export async function getL1ClaimDataScrollBridgeApi(l2BridgeInitiationContract, txHash, pageSize = 10) {
...
export async function getClaimDataScroll(adapterContract, txHash) {
...
export async function claimL1WithdrawScroll(claimInfo, signer) {
    ...
    const tx = await L1ScrollMessenger.relayMessageWithProof(ethers.getAddress(claimInfo.from), ethers.getAddress(claimInfo.to), BigInt(claimInfo.value), BigInt(claimInfo.nonce), ethers.hexlify(claimInfo.message), {
        batchIndex: BigInt(claimInfo.proof.batch_index),
        merkleProof: ethers.hexlify(claimInfo.proof.merkle_proof)
    });
    return tx;
}
export async function bridgeEVMLocalRootToL1(L2Adapter, signer, confirmations = 3) {
    // TODO
 ...
 * @returns
 */
export async function bridgeAZTECLocalRootToL1(aztecNode, L2AztecBridgeAdapter, L1AztecBridgeAdapter, provider, aztecWallet, sponsoredPaymentMethod, confirmations = 1) {
    const blockNumberOfRoot = await aztecNode.getBlockNumber(); //getBlockNumber();
    const PXE_L2Root = (await aztecNode.getBlock(blockNumberOfRoot))?.header.state.partial.noteHashTree.root;
    ...
    const messageBlockNumber = sendRootEffect?.l2BlockNumber; //await PXE.getBlockNumber(); // the blockNumber of when send_root_to_l1 settled onchain
    const contentHash = messageLeaf; //L2toL2Messages[0][0]
    const messageWitness = await computeL2ToL1MembershipWitness(aztecNode, messageBlockNumber, contentHash);
    const siblingPathArray = messageWitness.siblingPath.toFields().map((f) => f.toString());
    ...
    const args = [
        ...
        BigInt(messageBlockNumber), // hash to be the same block as the witness was retrieved since that is what the witness will be proved against
        messageWitness.leafIndex,
        siblingPathArray
    ];
    const waitFunc = async () => await waitForBlocksAztec(blocksToWait, aztecNode, isSandBox, L2AztecBridgeAdapter, aztecWallet);
    const refreshRootTx = await (await tryUntilItWorks(L1AztecBridgeAdapter, "getNewRootFromL2", args, waitFunc)).wait(confirmations);
...
export async function bridgeLocalRootToL1(l1Wallet, gigaBridge, L1Adapter, L2Adapter, isAztec, aztecNode, sponsoredPaymentMethodAZTEC, aztecWallet, confirmations = 3) {
    ...
    if (isAztec) {
        if (aztecNode === undefined) {
            throw new Error("aztecNode cant be undefined");
        }
        ...
        const { sendRootToL1Tx, refreshRootTx, PXE_L2Root } = await bridgeAZTECLocalRootToL1(aztecNode, L2Adapter, L1Adapter, l1Provider, aztecWallet, sponsoredPaymentMethodAZTEC, confirmations);
        const gigaRootPreBridge = await gigaBridge.gigaRoot();
        return { sendRootToL1Tx, sendRootToL1TxHash: sendRootToL1Tx.txHash };
    }
    else {
        const sendRootToL1Tx = await bridgeEVMLocalRootToL1(L2Adapter, l1Wallet, confirmations);
 ...
 * @returns
 */
export async function updateGigaRoot(gigaBridge, localRootProviders, confirmations = 1) {
    ...
    const isValidLocalRootProviders = await Promise.all(localRootProviders.map(async (localProviderAddr) => {
        ...
        const localRootProvider = L1AztecBridgeAdapter__factory.connect(localProviderAddr, provider);
        try {
    ...
    const refreshedNonceManager = new NonceManager(gigaBridge.runner);
    gigaBridge = gigaBridge.connect(refreshedNonceManager);
    const gigaRootUpdateTx = await (await gigaBridge.updateGigaRoot(validLocalRootProviders)).wait(confirmations);
 ...
 * @returns
 */
export async function sendGigaRoot(gigaBridge, gigaRootRecipients, allPayableGigaRootRecipients, confirmations = 1) {
    ...
    gigaBridge = gigaBridge.connect(new NonceManager(gigaBridge.runner));
    const sendGigaRootTx = await (await gigaBridge["sendGigaRoot(address[],uint256[])"](gigaRootRecipients, amounts, { value: totalEth })).wait(confirmations);
 ...
 * @returns
 */
export async function receiveGigaRootOnAztec(L2AztecBridgeAdapter, L1AztecBridgeAdapter, AztecWarpToad, sendGigaRootTx, // either get it from sendGigaRoot. or event scan for a specific gigaRoot with "NewGigaRootSentToAztec"
aztecNode, isSandBox, sponsoredPaymentMethod, aztecWallet) {
    ...
    if (isSandBox) {
        ...
        await L2AztecBridgeAdapter.methods.count(0n).send({ from: (await aztecWallet.getAccounts())[0].item }).wait();
        await L2AztecBridgeAdapter.methods.count(4n).send({ from: (await aztecWallet.getAccounts())[0].item }).wait();
    }
    else {
        console.warn("isSandBox is not set or detected. I hope ur indeed not on sandbox because it will break if u are!");
        await waitForBlocksAztec(blocksToWait, aztecNode, isSandBox, L2AztecBridgeAdapter, aztecWallet);
    }
    const receiveGigaRootTx = await L2AztecBridgeAdapter.methods.receive_giga_root(content_hash, index, AztecWarpToad.address).send({ fee: { paymentMethod: sponsoredPaymentMethod }, from: (await aztecWallet.getAccounts())[0].item }).wait({ timeout: 60 * 60 * 12 });
    return { receiveGigaRootTx };
}
export async function waitForBlocksAztec(blocksToWait, aztecNode, isSandBox, L2AztecBridgeAdapter, aztecWallet) {
...
export async function tryUntilItWorks(contract, funcName, funcArgs, waitFunc) {
    let works = false;
    while (works === false) {
        try {
...
export function parseEventFromTx(tx, contract, eventName) {
...
export async function receiveGigaRootOnEvmL2(L2Adapter, gigaRootSent) {
    const provider = L2Adapter.runner?.provider;
    const chainId = (await provider?.getNetwork()).chainId;
    switch (chainId) {
        case chainIds.scroll.testnet:
...
export async function receiveGigaRootOnL2(L1Adapter, L2Adapter, L2WarpToad, sendGigaRootTx, gigaRootSent, 
// aztec
isAztec, aztecWallet, isSandBox, aztecNode, sponsoredPaymentMethod) {
    if (isAztec) {
        if (aztecNode === undefined) {
            throw new Error("isSandBox cant be undefined");
        }
        ...
        const { receiveGigaRootTx } = await receiveGigaRootOnAztec(L2Adapter, L1Adapter, L2WarpToad, sendGigaRootTx, aztecNode, isSandBox, sponsoredPaymentMethod, aztecWallet);
        const gigaRootOnAztec = await L2WarpToad?.methods.get_giga_root().simulate({ from: (await aztecWallet.getAccounts())[0].item });
...
export function parseMultipleEventsFromTx(tx, contract, eventName) {
 ...
 */
export async function bridgeBetweenL1AndL2(l1Wallet, L1Adapter, gigaBridge, L2Adapter, L2WarpToad, localRootProviders, payableLocalRootProviders, aztecInputs) {
    const l1ChainId = (await (l1Wallet.provider?.getNetwork()))?.chainId;
    const isSandBox = l1ChainId === 31337n;
    ...
    // aztec inputs
    aztecInputs.isAztec, aztecInputs.aztecNode, aztecInputs.sponsoredPaymentMethod, aztecInputs.aztecWallet, confirmations);
    ...
    const { gigaRootUpdateTx, gigaRootUpdateTxHash } = await updateGigaRoot(gigaBridge, localRootProviders, confirmations);
    console.log(`GigaRoot is updated! At tx hash: ${gigaRootUpdateTxHash}`);
    console.log(`initiating gigaRoot bridging to the L2's`);
    const { sendGigaRootTx, sendGigaRootTxHash, gigaRootSent } = await sendGigaRoot(gigaBridge, localRootProviders, payableLocalRootProviders, confirmations);
    ...
    const { receiveGigaRootTx, receiveGigaRootTxHash } = await receiveGigaRootOnL2(L1Adapter, L2Adapter, L2WarpToad, sendGigaRootTx, gigaRootSent, aztecInputs.isAztec, aztecInputs.aztecWallet, isSandBox, aztecInputs.aztecNode, aztecInputs.sponsoredPaymentMethod);
...
//# sourceMappingURL=bridging.js.map