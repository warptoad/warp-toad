...
async function main() {
    ...
    console.log("\n=== Checking values ===");
    console.log("Is gigaRoot valid?", await l1WarpToad.read.isValidGigaRoot([BigInt(gigaRoot)]));
    console.log("Is localRoot valid?", await l1WarpToad.read.isValidLocalRoot([BigInt(localRoot)]));
    console.log("Is nullifier used?", await l1WarpToad.read.nullifiers([BigInt(nullifier)]));

    console.log("\n=== Attempting mint ===");
    try {
        // Simulate the call first to surface the revert reason
        await l1WarpToad.simulate.mint([
            BigInt(nullifier),
            BigInt(amount),
            BigInt(gigaRoot),
            BigInt(localRoot),
