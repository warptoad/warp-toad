...
export  async function deployL2AztecBridgeAdapter(L1AztecBridgeAdapter:ethers.AddressLike,deployerWallet:AztecWallet, sponsoredPaymentMethod:SponsoredFeePaymentMethod|undefined) {
    ...
    const L2AztecBridgeAdapter = await Contract.deploy(deployerWallet, L2AztecBridgeAdapterContractArtifact, constructorArgs).send({ fee: { paymentMethod: sponsoredPaymentMethod }, from:deployerWallet.getAddress() }).deployed({timeout:60*60*12}) as L2AztecBridgeAdapterContract;
