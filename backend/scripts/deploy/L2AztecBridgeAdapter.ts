...
export  async function deployL2AztecBridgeAdapter(L1AztecBridgeAdapter:ethers.AddressLike,deployerWallet:AztecWallet) {
    ...
    const L2AztecBridgeAdapter = await Contract.deploy(deployerWallet, L2AztecBridgeAdapterContractArtifact, constructorArgs).send().deployed({timeout:60*60*12}) as L2AztecBridgeAdapterContract;
