...
export async function getAztecTestAccountNoEnv(chainId: bigint, nodeUrl: string) {
    ...
    if (chainId == 31337n) {
        ...
        await wallet.createSchnorrInitializerlessAccount(testAccountData.secret, testAccountData.salt, testAccountData.signingKey);
        return wallet
    } else {
        console.warn("assuming ur on testnet since chainId is NOT 31337")
        await wallet.createSchnorrAccount(testAccountData.secret, testAccountData.salt, testAccountData.signingKey);
        return wallet
    }
}
