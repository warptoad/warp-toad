...
import os from 'os';
import { WarpToadCoreContractArtifact } from '../contracts/aztec/WarpToadCore/src/artifacts/WarpToadCore'; //"@warp-toad/backend/aztec/WarpToadCore" //'../contracts/aztec/WarpToadCore/src/artifacts/WarpToadCore'
import { L2AztecBridgeAdapterContractArtifact } from "../contracts/aztec/L2AztecBridgeAdapter/src/artifacts/L2AztecBridgeAdapter"; //"@warp-toad/backend/aztec/L2AztecBridgeAdapter"//'../contracts/aztec/L2AztecBridgeAdapter/src/artifacts/L2AztecBridgeAdapter'
...
import { calculateFeeFactor, createProof, getProofInputs } from "../scripts/lib/proving"; //"@warp-toad/backend/proving";
import { 
//sendGigaRoot, bridgeAZTECLocalRootToL1, parseEventFromTx, updateGigaRoot, receiveGigaRootOnAztec, 
bridgeBetweenL1AndL2 } from "../scripts/lib/bridging"; //"@warp-toad/backend/bridging";
...
describe("AztecWarpToad", function () {
    ...
    async function deployL2AztecBridgeAdapterContract(aztecDeployerWallet, constructorArgs) {
        return await Contract.deploy(aztecDeployerWallet, L2AztecBridgeAdapterContractArtifact, constructorArgs).send({ from: (await aztecDeployerWallet.getAccounts())[0].item }).deployed();
    }
    async function deploy() {
        ...
        await AztecWarpToad.methods.initialize(L2AztecBridgeAdapter.address, L1AztecBridgeAdapter.target).send({ from: (await aztecDeployerWallet.getAccounts())[0].item }).wait(); // all other warptoad initializations will look like this
    ...
    describe("burnAztecMintEvm", function () {
        it("Should burn and verify with the evm circuit", async function () {
            ...
            const balancePreBurn = await AztecWarpToad.methods.balance_of(aztecDeployerAddress).simulate({ from: aztecDeployerAddress });
            const { chainId: chainIdEvmProvider } = await provider.getNetwork();
            const aztecVersion = (await AZTEC_NODE.getNodeInfo()).rollupVersion;
            const chainIdAztecFromContract = hre.ethers.toBigInt(await AztecWarpToad.methods.get_chain_id_unconstrained(aztecVersion).simulate({ from: aztecDeployerAddress }));
            ...
            const burnTx1 = await AztecWarpToad.methods.burn(commitmentPreImg1.amount, commitmentPreImg1.destination_chain_id, commitmentPreImg1.secret, commitmentPreImg1.nullifier_preimg).send({ from: aztecDeployerAddress }).wait();
            const balancePostBurn = await AztecWarpToad.methods.balance_of(aztecDeployerAddress).simulate({ from: aztecDeployerAddress });
            ...
            //     gigaRootRecipients
            // )
            await bridgeBetweenL1AndL2(evmRelayer, L1AztecBridgeAdapter, gigaBridge, L2AztecBridgeAdapter, AztecWarpToad, localRootProviders, [], // no payable root providers (only aztec!)
            {
                isAztec: true,
                PXE: PXE,
            ...
            const newGigaRootFromL2 = await AztecWarpToad.methods.get_giga_root().simulate({ from: aztecDeployerAddress });
            ...
            const burnTx2 = await AztecWarpToad.methods.burn(commitmentPreImg2.amount, commitmentPreImg2.destination_chain_id, commitmentPreImg2.secret, commitmentPreImg2.nullifier_preimg).send({ from: aztecDeployerAddress }).wait();
            await L2AztecBridgeAdapter.methods.count(463n).send({ from: aztecDeployerAddress }).wait();
            ...
            // -------------mint-----------------------------------
            console.log("mint!");
            const proofInputs = await getProofInputs(gigaBridge, L1WarpToad, AztecWarpToad, amountToBurn1, feeFactor, priorityFee, maxFee, await evmRelayer.getAddress(), await evmRecipient.getAddress(), commitmentPreImg1.nullifier_preimg, commitmentPreImg1.secret, aztecDeployer, PXE, AZTEC_NODE);
...
// }
//# sourceMappingURL=testAztecToL1.js.map