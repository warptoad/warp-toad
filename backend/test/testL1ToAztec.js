...
import { calculateFeeFactor, getMerkleData } from "../scripts/lib/proving"; //"@warp-toad/backend/proving";
import { bridgeBetweenL1AndL2 } from "../scripts/lib/bridging"; //"@warp-toad/backend/bridging";
...
describe("AztecWarpToad", function () {
    ...
    describe("burnAztecMintEvm", function () {
        it("Should burn and verify with the evm circuit", async function () {
            ...
            const mintTx = await AztecWarpToad.methods.mint_giga_root_evm(commitmentPreImg1.amount, commitmentPreImg1.secret, commitmentPreImg1.nullifier_preimg, (await aztecDeployer.getAccounts())[0].item, aztecMerkleData1.blockNumber, aztecMerkleData1.originLocalRoot, aztecMerkleData1.gigaMerkleData, // no way i am gonna spend time getting this type right >:(
            aztecMerkleData1.evmMerkleData).send({ from: (await aztecDeployer.getAccounts())[0].item }).wait();
            // check mint tx
            console.log("TODO balance_of!!!!!!!!");
            ...
            await AztecWarpToad.methods.mint_giga_root_evm(commitmentPreImg2.amount, commitmentPreImg2.secret, commitmentPreImg2.nullifier_preimg, (await aztecDeployer.getAccounts())[0].item, aztecMerkleData2.blockNumber, aztecMerkleData2.originLocalRoot, aztecMerkleData2.gigaMerkleData, // no way i am gonna spend time getting this type right >:(
            aztecMerkleData2.evmMerkleData).send({ from: (await aztecDeployer.getAccounts())[0].item }).wait();
...
//# sourceMappingURL=testL1ToAztec.js.map